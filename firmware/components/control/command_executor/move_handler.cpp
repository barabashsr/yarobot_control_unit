 ...
 * @brief MOVE command handler implementation
 * @author YaRobot Team
 * @date 2025
 */
...
static const char* TAG = "MOVE_HDL";
...
esp_err_t handle_move(const ParsedCommand* cmd, char* response, size_t resp_len)
{
    ...
    // MOVE requires at least 1 parameter (position)
    if (cmd->param_count < 1) {
        ESP_LOGD(TAG, "No position parameter");
    ...
    // Execute move (AC1, AC2)
    esp_err_t ret = controller->moveAbsolute(static_cast<uint8_t>(axis_id), position, velocity);

    // Map return codes to responses (AC6, AC7)
    if (ret == ESP_OK) {
        return format_ok(response, resp_len);
    } else if (ret == ESP_ERR_INVALID_STATE) {
        // Axis not enabled (AC6)
        return format_error(response, resp_len, ERR_AXIS_NOT_ENABLED, MSG_AXIS_NOT_ENABLED);
    } else if (ret == ESP_ERR_INVALID_ARG) {
        // Position limit exceeded (AC7)
        return format_error(response, resp_len, ERR_POSITION_LIMIT, MSG_POSITION_LIMIT);
    } else {
        // Unknown error
        ESP_LOGW(TAG, "Unexpected error from moveAbsolute: 0x%x", ret);
...
esp_err_t move_handler_register(void)
{
    ...
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "Registered MOVE handler");
    } else {
        ESP_LOGE(TAG, "Failed to register MOVE handler: 0x%x", ret);
    }
    return ret;
}
